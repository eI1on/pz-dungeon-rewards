...
DungeonRewards.VERSION = "1"
DungeonRewards.DATA_VERSION = 1
DungeonRewards.DATA_DIR = "Dungeon Rewards"
DungeonRewards.CONFIG_FILE = DungeonRewards.DATA_DIR .. "/presets_export.json"
...
function Shared.LoadPresetsFromFile()
	return require("DungeonRewards/Persistence").loadPresetImportFile()
end

function Shared.SavePresetsToFile(presets)
	return require("DungeonRewards/Persistence").savePresetExportFile(presets)
end
...
function Shared.RollRewards(preset)
	...
	for rollIndex = 1, count do
		...
		local pick = type(ZombRand) == "function" and (ZombRand(0, math.floor(total * 10000)) / 10000)
