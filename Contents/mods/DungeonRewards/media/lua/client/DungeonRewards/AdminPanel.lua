...
function AdminPanel:createChildren()
	...
	self.removeContainerBtn =
		addButton(self, 0, 0, 130, getText("IGUI_DRewards_RemoveContainer"), "REMOVE_CONTAINER", "danger")
...
function AdminPanel:layoutChildren()
	...
	setBounds(
		self.resetClaimsBtn,
		pad + smallButtonW + C.LAYOUT.GAP,
		assignY + C.CTRL.FIELD_H + C.LAYOUT.GAP,
		leftW - smallButtonW - C.LAYOUT.GAP,
		C.CTRL.BUTTON_H
	)
	setBounds(
		self.removeContainerBtn,
		pad,
		assignY + C.CTRL.FIELD_H + C.LAYOUT.GAP + C.CTRL.BUTTON_H + C.LAYOUT.GAP,
		leftW,
		C.CTRL.BUTTON_H
	)
	...
	setBounds(
		...
		editorX + typeW + C.LAYOUT.GAP,
		fieldY,
		math.max(92, editorW - typeW - C.LAYOUT.GAP),
		C.CTRL.FIELD_H
	)
	fieldY = fieldY + 52
...
function AdminPanel:loadRewardIntoEditor(reward)
	reward = reward
		or Shared.NormalizeReward({ type = "item", item = "Base.WaterBottleFull", count = 1, weight = 10 }, 1)
...
function AdminPanel:buildRewardFromEditor()
	...
	local reward = {
		id = self.workingRewards[self.selectedRewardIndex] and self.workingRewards[self.selectedRewardIndex].id
			or Shared.GenerateID("reward"),
		type = rewardType,
...
function AdminPanel:exportPresets()
	...
	self:setStatus(
		success and getText("IGUI_DRewards_StatusExported") or getText("IGUI_DRewards_StatusExportFailed"),
...
function AdminPanel.onTickChanged(target, index, selected) end

function AdminPanel.onPresetComboChanged(target, combo) end
...
function AdminPanel.drawPresetItem(list, y, item, alt)
	...
	local selected = list.selected == item.index
		or (list.parentPanel and list.parentPanel.selectedPresetId == preset.id)
	...
	list:drawText(
		trimToWidth(UIFont.Small, preset.name, contentW - 12),
		8,
		y + 4,
		C.COLORS.TEXT.r,
		C.COLORS.TEXT.g,
...
function AdminPanel.drawContainerItem(list, y, item, alt)
	...
	local selected = list.selected == item.index
		or (list.parentPanel and list.parentPanel.selectedContainerKey == container.key)
	local bg = selected and C.COLORS.SELECTED or (alt and C.COLORS.ALT or C.COLORS.FIELD)
	local contentW = list:getWidth() - C.LIST.SCROLL_W
	list:drawRect(0, y, list:getWidth(), list.itemheight - 1, bg.a, bg.r, bg.g, bg.b)
	list:drawText(
		trimToWidth(UIFont.Small, container.name or container.key, contentW - 12),
		8,
		y + 4,
		C.COLORS.TEXT.r,
		C.COLORS.TEXT.g,
	...
	list:drawText(
		trimToWidth(
			...
			string.format(
				...
				container.y or 0,
				container.z or 0,
				container.presetId ~= "" and container.presetId or "No preset"
			),
			contentW - 12
		),
		8,
...
function AdminPanel.drawRewardItem(list, y, item, alt)
	...
	list:drawText(
		trimToWidth(
			...
			string.format(
				"%s | %s",
				tostring(reward.type or "?"):upper(),
				reward.title or Shared.GetRewardSummary(reward)
			),
			w - 94
		),
		8,
	...
	list:drawText(
		trimToWidth(UIFont.Small, rewardTargetText(reward) .. " " .. rewardValueText(reward), w - 110),
		8,
		y + 23,
		C.COLORS.MUTED.r,
		C.COLORS.MUTED.g,
	...
	list:drawTextRight(
		string.format("w%s %d%%", tostring(reward.weight or 0), pct),
		w - 8,
		y + 13,
		C.COLORS.GOLD.r,
		C.COLORS.GOLD.g,
...
function AdminPanel:drawLabel(text, x, y, w)
	self:drawText(
		trimToWidth(UIFont.Small, text, w or 200),
		x,
		y,
		C.COLORS.MUTED.r,
		C.COLORS.MUTED.g,
...
function AdminPanel:drawLabels()
	self:drawSectionTitle(
		getText("IGUI_DRewards_Presets"),
		self.presetsList:getX(),
		self.presetsList:getY() - FONT_MEDIUM - 8,
		self.presetsList:getWidth()
	)
	self:drawSectionTitle(
		getText("IGUI_DRewards_Containers"),
		self.containersList:getX(),
		self.containersList:getY() - FONT_MEDIUM - 8,
		self.containersList:getWidth()
	)
	local selectedContainer = self:getSelectedContainer()
	local selectedText = selectedContainer
			and string.format(
				"%s %d,%d,%d",
				getText("IGUI_DRewards_SelectedContainer"),
				selectedContainer.x or 0,
				selectedContainer.y or 0,
				selectedContainer.z or 0
			)
		or getText("IGUI_DRewards_NoContainerSelected")
	self:drawLabel(
		selectedText,
		self.assignCombo:getX(),
		self.assignCombo:getY() - FONT_SMALL - 4,
		self.assignCombo:getWidth()
	)
	self:drawSectionTitle(
		getText("IGUI_DRewards_PresetEditor"),
		self.presetIdEntry:getX(),
		C.LAYOUT.TOP,
		self.width - self.presetIdEntry:getX() - C.LAYOUT.PAD
	)
	self:drawLabel(
		getText("IGUI_DRewards_PresetId"),
		self.presetIdEntry:getX(),
		self.presetIdEntry:getY() - FONT_SMALL - 4,
		self.presetIdEntry:getWidth()
	)
	self:drawLabel(
		getText("IGUI_DRewards_PresetName"),
		self.presetNameEntry:getX(),
		self.presetNameEntry:getY() - FONT_SMALL - 4,
		self.presetNameEntry:getWidth()
	)
	self:drawLabel(
		getText("IGUI_DRewards_Rolls"),
		self.rollCountEntry:getX(),
		self.rollCountEntry:getY() - FONT_SMALL - 4,
		self.rollCountEntry:getWidth()
	)
	self:drawLabel(
		getText("IGUI_DRewards_Description"),
		self.descriptionEntry:getX(),
		self.descriptionEntry:getY() - FONT_SMALL - 4,
		self.descriptionEntry:getWidth()
	)
	self:drawSectionTitle(
		getText("IGUI_DRewards_Rewards"),
		self.rewardsList:getX(),
		self.rewardsList:getY() - FONT_MEDIUM - 8,
		self.rewardsList:getWidth()
	)
	self:drawSectionTitle(
		getText("IGUI_DRewards_RewardEditor"),
		self.rewardTypeCombo:getX(),
		self.rewardsList:getY() - FONT_MEDIUM - 8,
		self.width - self.rewardTypeCombo:getX() - C.LAYOUT.PAD
	)
	self:drawLabel(
		getText("IGUI_DRewards_RewardType"),
		self.rewardTypeCombo:getX(),
		self.rewardTypeCombo:getY() - FONT_SMALL - 4,
		self.rewardTypeCombo:getWidth()
	)
	self:drawLabel(
		getText("IGUI_DRewards_RewardTitle"),
		self.rewardTitleEntry:getX(),
		self.rewardTitleEntry:getY() - FONT_SMALL - 4,
		self.rewardTitleEntry:getWidth()
	)
	...
	self:drawLabel(
		targetLabel,
		self.rewardTargetEntry:getX(),
		self.rewardTargetEntry:getY() - FONT_SMALL - 4,
		self.rewardTargetEntry:getWidth()
	)
	self:drawLabel(
		rewardType == "xp" and getText("IGUI_DRewards_Amount") or getText("IGUI_DRewards_Count"),
		self.rewardCountEntry:getX(),
		self.rewardCountEntry:getY() - FONT_SMALL - 4,
		self.rewardCountEntry:getWidth()
	)
	self:drawLabel(
		getText("IGUI_DRewards_Weight"),
		self.rewardWeightEntry:getX(),
		self.rewardWeightEntry:getY() - FONT_SMALL - 4,
		self.rewardWeightEntry:getWidth()
	)
end
...
function AdminPanel:render()
	...
	local T = Theme.colors
	local color = self.statusLevel == "error" and T.danger or (self.statusLevel == "warning" and T.warning or T.success)
